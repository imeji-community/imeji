...
import de.mpg.escidoc.services.exportmanager.Export.ExportFormatTypes;
...
public class AlbumSession
{
    ...
    public AlbumSession()
    {
        active = new AlbumVO();
        current = new AlbumVO();
        myAlbums = new AlbumListVO(new ArrayList<AlbumVO>(), new AlbumListParameters(null, SortParameterType.LAST_MODIFICATION_DATE, OrderParameterType.DESCENDING, 10, 1, null, null), HandlerType.FILTER);
        published = new AlbumListVO(new ArrayList<AlbumVO>(), new AlbumListParameters(null, SortParameterType.LAST_MODIFICATION_DATE,  OrderParameterType.DESCENDING, 10, 1, null, null), HandlerType.SEARCH);
    ...
    {
        myAlbums = albumsListController.retrieve(myAlbums, sessionBean.getUserHandle());
        published = albumsListController.retrieve(published, sessionBean.getUserHandle());
    }
    ...
    public String getInitCurrentAlbum()
    {
                ...
                && !"".equals(request.getParameter("album")))
        {
            try 
            {
				current = albumController.retrieve((String)request.getParameter("album"), sessionBean.getUserHandle());
			} 
            catch (Exception e) 
            {
    ...
    public String getInitActiveAlbum()
    {
                ...
                && !"".equals(request.getParameter("selection")))
        {
            try 
            {
				active = albumController.retrieve((String)request.getParameter("selection"), sessionBean.getUserHandle());
			} 
            catch (Exception e) 
            {
                ...
                && active.getState().equals(State.PENDING))
        {
            ...
            try 
            {
				active = albumController.retrieve(active.getVersion().getObjectId(), sessionBean.getUserHandle());
			} 
            catch (Exception e) 
            {
...
//    	                navigation.getConfirmationUrl() + "/export/" + current.getVersion().getObjectId()+"?agree=1");
	...
	public List<AlbumVO> getSelectedAlbums() 
	{
        ...
        for (int i = 0; i < myAlbums.getList().size(); i++)
        {
            if (myAlbums.getList().get(i).isSelected())
            {
                selectedAlbums.add(myAlbums.getList().get(i));
            }
        }
