...
public abstract class ImejiController
{
    ...
    /*
     * private Map<SearchCriterion, ImejiQueryVariable> createOntology(List<SearchCriterion> scList,
     * Map<SearchCriterion, ImejiQueryVariable> map, Map<ImejiNamespaces, ImejiQueryVariable> currentMap,
     * List<ImejiQueryVariable> roots) { if(scList!=null && scList.size()>0) { for(SearchCriterion sc : scList) {
     * if(!sc.getNamespace().equals(ImejiNamespaces.ID_URI)) { if(sc.getNamespace()!=null) { ImejiNamespaces ns =
     * sc.getNamespace(); ImejiQueryVariable child = null; while (ns!= null) { ImejiQueryVariable qv;
     * if(!currentMap.containsKey(ns) || (currentMap.containsKey(ns) && ns.isCollectionType())) {
     * List<ImejiQueryVariable> qvList = new ArrayList<ImejiQueryVariable>(); if(child!=null) qvList.add(child); qv =
     * new ImejiQueryVariable(ns, qvList); map.put(sc, qv); currentMap.put(ns, qv); } else { qv = currentMap.get(ns);
     * if(child!=null && !qv.getChildren().contains(child)) qv.getChildren().add(child); } child = qv; ns =
    ...
    private static void createOntology(Map<ImejiNamespaces, ImejiQueryVariable> variableMap,
            List<SearchCriterion> scList, List<ImejiQueryVariable> currentRoots)
    {
        if (scList != null)
        {
            for (SearchCriterion sc : scList)
    ...
    public static String createFilter(List<SearchCriterion> scList, Map<ImejiNamespaces, ImejiQueryVariable> variableMap)
    {
        ...
        for (SearchCriterion sc : scList)
        {
            ...
            if (sc.getValue() != null && sc.getNamespace() != null)
            {
                ...
                {
                    filter += "regex(" + variableMap.get(sc.getNamespace()).getVariable() + ", '" + sc.getValue()
                            + "', 'i')";
                }
                else if (sc.getFilterType().equals(Filtertype.EQUALS))
                {
     ...
     * In development!
     */
    public String createQuery2(List<SearchCriterion> scList, SortCriterion sortCriterion, String type, int limit,
            int offset) throws Exception
    {
        String query = "";
        query += getSpecificQuery();
         ...
         * for(List<SearchCriterion> subList : scList) { if (scList.indexOf(subList)>0 && subList.size()>0) {
         * if(subList.get(0).getOperator().equals(SearchCriterion.Operator.AND)) filter += " && "; else
         * if(subList.get(0).getOperator().equals(SearchCriterion.Operator.OR)) filter += " || "; } filter+="(";
         * for(SearchCriterion sc : subList) { if (subList.indexOf(sc) > 0) {
         * if(sc.getOperator().equals(SearchCriterion.Operator.AND)) filter += " && "; else
         * if(sc.getOperator().equals(SearchCriterion.Operator.OR)) filter += " || "; } if(sc.getValue()!=null) {
         * if(sc.getFilterType().equals(Filtertype.URI) && !sc.getNamespace().equals(ImejiNamespaces.ID_URI)) { filter
         * += map.get(subList).get(sc.getNamespace()).getVariable() + "=<" + sc.getValue() + ">"; } else
         * if(sc.getFilterType().equals(Filtertype.URI) && sc.getNamespace().equals(ImejiNamespaces.ID_URI)) { filter +=
         * "?s" + "=<" + sc.getValue() + ">"; } else if(sc.getFilterType().equals(Filtertype.REGEX)) { filter +=
         * "regex(" + map.get(subList).get(sc.getNamespace()).getVariable() + ", '" + sc.getValue() + "', 'i')"; } else
         * if(sc.getFilterType().equals(Filtertype.EQUALS)) { filter +=
         * map.get(subList).get(sc.getNamespace()).getVariable() + "='" + sc.getValue() + "'"; } else
         * if(sc.getFilterType().equals(Filtertype.BOUND)) { filter += "bound(" +
         * map.get(subList).get(sc.getNamespace()).getVariable() + ")=" + sc.getValue() + ""; } } j++; } filter+=")"; }
         * filter+=")"; }
         */
        String specificFilter = ". FILTER(" + getSpecificFilter() + ")";
        // Add sort criterion
        ...
        // If inverse search add filter to MINUS part
        /*
         * if(inverse) { query = query.substring(0, query.lastIndexOf("}")); query += filter + "}"; } else { query +=
         * filter; }
         */
        String completeQuery = "SELECT DISTINCT ?s WHERE { ?s a <" + type + "> " + query + specificFilter + " } "
                + sortQuery + limitString + " OFFSET " + offset;
    ...
    private static String createSubQuery2(List<ImejiQueryVariable> qvList, VarCounter y, String oldSubjectVar,
            boolean inverse)
    {
        String subquery = "";
        if (qvList != null)
