...
<jsp:output doctype-root-element="html"
	...
	<f:view locale="#{SessionBean.locale}" xmlns:e="http://www.escidoc.de/jsf">
		...
		<html xmlns="http://www.w3.org/1999/xhtml">
			...
			<h:outputText id="txtDetailInit" value="#{Detail.init}" styleClass="noDisplay" />
			
			<h:form id="formComparison">
			...
			<h:inputHidden id="offset"></h:inputHidden>
				...
				<!-- begin: content section (including elements that visualy belong to the header (breadcrumb, headline, subheader and content menu)) -->
						...
						<div class="small_marginLIncl subHeaderSection">
							...
							<!-- content menu starts here -->
								...
								<!-- content menu upper line starts here -->
									<h:outputText id="txtContentMenu" styleClass="free_area0" value="Show"/>
								<!-- content menu upper line ends here -->
								</div>
								<h:panelGroup layout="block" styleClass="free_area0 sub action">
								<!-- content menu lower line starts here -->
									<h:outputLink id="lnkAllPics1" styleClass="free_area0" value="#{Navigation.applicationUrl}person/#{Detail.item.mdRecords.screen.mdMap['face.person.identifier'].simpleValue}/#{Navigation.defaultBrowsingKeepShow}" rendered="#{Detail.item.type == 'face'}">
										<h:outputText value="#{lbl.show_persons_images}"/>
									</h:outputLink>
									<!--<h:outputText styleClass="seperator" rendered="#{Detail.item.type == 'face-item'}" />

									<h:outputLink id="lnkAddToAlbum" styleClass="free_area0" value="#{Navigation.applicationUrl}action/#{AlbumSession.active.latestVersion.objectId}/add/#{Detail.item.item.objid}" rendered="#{(!Detail.item.inAlbum) and (AlbumSession.active.latestVersion.objectId != null) and (AlbumSession.active.state == 'PENDING')}">
										<h:outputText value="Add to Album"/>
									</h:outputLink>
									<h:outputText styleClass="seperator" rendered="#{(!Detail.item.inAlbum) and (AlbumSession.active.latestVersion.objectId != null) and (AlbumSession.active.state == 'PENDING')}" />

									<h:outputLink id="lnkRemoveFromAlbum" styleClass="free_area0" rendered="#{(AlbumSession.current.size != '0') and (AlbumSession.current.state == 'PENDING')}" value="#{Navigation.applicationUrl}action/#{AlbumSession.current.latestVersion.objectId}/remove/#{Detail.item.item.objid}/back">
					...
					<h:panelGroup layout="block" styleClass="full_area0 fullItem">
						...
						<jsp:directive.include file="picturesPages/ui/AlternativeImage.jspf"/>
					</h:panelGroup>
