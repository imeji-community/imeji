...
public class ExportManager 
{
	...
	public SearchResult search(List<SearchCriterion> scList)
	{
		...
		else if ("image".equals(searchType))
		{
			...
			{
				result = itemController.searchImagesInContainer(ObjectHelper.getURI(CollectionImeji.class, collectionId), scList, null, maximumNumberOfRecords, 0);
			}
			else if (albumId != null)
			{
				result = itemController.searchImagesInContainer(ObjectHelper.getURI(Album.class, albumId), scList, null, maximumNumberOfRecords, 0);
			}
			else
			{
