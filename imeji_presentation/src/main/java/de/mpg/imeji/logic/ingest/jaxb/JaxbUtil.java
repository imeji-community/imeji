...
/**
 * @author http://www.torsten-horn.de/techdocs/java-xml-jaxb.htm
 * modified by hnguyen
 * 
 */
public class JaxbUtil {

	public static <T> T unmarshal(String xsdFile, String xmlFile,
			Class<T> clss) throws JAXBException, SAXException {
		SchemaFactory schemaFactory = SchemaFactory.newInstance(XMLConstants.W3C_XML_SCHEMA_NS_URI);
		Schema schema = (xsdFile == null || xsdFile.trim().length() == 0) ? null : schemaFactory.newSchema(new File(xsdFile));
		JAXBContext jaxbContext = JAXBContext.newInstance(clss.getPackage().getName());
		return unmarshal(jaxbContext, schema, xmlFile, clss);
	}

	public static <T> T unmarshal(JAXBContext jaxbContext, Schema schema, String xmlFile, Class<T> clss) throws JAXBException {
	...
	public static void marshal(JAXBContext jaxbContext, Schema schema, String xmlFile, Object jaxbElement) throws JAXBException {
