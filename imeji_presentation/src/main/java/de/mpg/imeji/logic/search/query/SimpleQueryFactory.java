...
public class SimpleQueryFactory 
{
	...
	public static String getSearchElement(SearchCriterion sc)
	{
		...
		{
			//slow
			searchQuery = ". OPTIONAL {?s <http://imeji.org/terms/metadataSet> ?mds . OPTIONAL {?mds <http://imeji.org/terms/metadata> ?md .OPTIONAL{ ?md <http://imeji.org/terms/complexTypes> ?type . OPTIONAL {?md <" + sc.getNamespace().getNs() + "> ?el }}}}";
			//fast (not tested)
			//searchQuery = ". ?s <http://imeji.org/terms/metadataSet> ?mds . ?mds <http://imeji.org/terms/metadata> ?md . ?md <http://imeji.org/terms/complexTypes> ?type .  ?md <" + sc.getNamespace().getNs() + "> ?el ";
		}
		...
		{
			searchQuery += ". ?s <http://imeji.org/terms/metadataSet> ?mds . ?mds <http://imeji.org/terms/metadata> ?md  . ?md <http://imeji.org/terms/metadata/person> ?p . ?p  <" +  sc.getNamespace().getNs() + "> ?el ";
		}
		else
		{
			//slow
			//searchQuery = ". OPTIONAL {?s <http://imeji.org/terms/metadataSet> ?mds . OPTIONAL {?mds <http://imeji.org/terms/metadata> ?md  . OPTIONAL {?md <" + sc.getNamespace().getNs() + "> ?el }}}";
			//fast for simple metadata search
			searchQuery = ". ?s <http://imeji.org/terms/metadataSet> ?mds . ?mds <http://imeji.org/terms/metadata> ?md  . ?md <" + sc.getNamespace().getNs() + "> ?el ";
		}
			...
			else if (sc.getValue().startsWith("\"") && sc.getValue().endsWith("\""))
			{
				filter +=  variable + "='" + sc.getValue().replaceAll("\"", "")+ "'";
					...
					(sc.getNamespace().equals(SearchIndexes.ID_URI) 
							|| sc.getNamespace().equals(SearchIndexes.IMAGE_METADATA_TYPE)
							|| sc.getNamespace().equals(SearchIndexes.IMAGE_METADATA_STATEMENT)))
			{
				filter +=  variable + "=<" + sc.getValue() + ">";
			}
