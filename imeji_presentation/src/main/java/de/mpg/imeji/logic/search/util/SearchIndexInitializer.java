...
public class SearchIndexInitializer
{
    ...
    private static Map<String, SearchIndex> initBasisIndexes()
    {
         ...
         * Properties indexes
         */
        indexes = put(indexes, new SearchIndex(SearchIndex.names.creator.name(),
                "http://imeji.org/terms/createdBy"));
        indexes = put(indexes, new SearchIndex(SearchIndex.names.editor.name(),
                "http://imeji.org/terms/modifiedBy"));
        indexes = put(indexes, new SearchIndex(SearchIndex.names.created.name(),
                "http://purl.org/dc/terms/created"));
        indexes = put(indexes, new SearchIndex(SearchIndex.names.modified.name(),
                "http://purl.org/dc/terms/modified"));
        indexes = put(indexes, new SearchIndex(SearchIndex.names.status.name(),
                "http://imeji.org/terms/status"));
        /**
         * Grant indexes
         */
        indexes = put(indexes, new SearchIndex(SearchIndex.names.grant.name(),
         ...
         * Item Indexes
         */
        indexes = put(indexes, new SearchIndex(SearchIndex.names.filename.name(),
         ...
         * Collection indexes
         */
        indexes = put(indexes, new SearchIndex(SearchIndex.names.col.name(),
                "http://imeji.org/terms/collection"));
        indexes = put(indexes, new SearchIndex(SearchIndex.names.prof.name(),
                "http://imeji.org/terms/mdprofile", indexes.get(SearchIndex.names.col.name())));
        /**
        ...
        indexes = put(
                indexes,
                new SearchIndex(SearchIndex.names.cont_description.name(),
                        "http://purl.org/dc/elements/1.1/description", indexes.get(SearchIndex.names.cont_md
                                .name())));
        indexes = put(
                indexes,
                new SearchIndex(SearchIndex.names.cont_person.name(),
                        "http://purl.org/escidoc/metadata/terms/0.1/creator", indexes
                                .get(SearchIndex.names.cont_md.name())));
        indexes = put(
                indexes,
        ...
        indexes = put(
                indexes,
                new SearchIndex(SearchIndex.names.cont_person_org_name.name(),
                        "http://purl.org/dc/elements/1.1/title", indexes
                                .get(SearchIndex.names.cont_person_org.name())));
        /**
         * Metadata profile indexes
         */
        indexes = put(indexes, new SearchIndex(SearchIndex.names.profile.name(),
                "http://imeji.org/terms/mdprofile"));
        /**
         * Image Metadata indexes
         */
        indexes = put(indexes, new SearchIndex(SearchIndex.names.md.name(),
                "http://imeji.org/terms/metadata"));
        indexes = put(indexes, new SearchIndex(SearchIndex.names.statement.name(),
    ...
    {
        Map<String, SearchIndex> indexes = new HashMap<String, SearchIndex>();
        indexes = put(indexes, new SearchIndex(SearchIndex.names.text.name(),
                "http://imeji.org/terms/text", indexes.get(SearchIndex.names.md.name())));
        indexes = put(indexes, new SearchIndex(SearchIndex.names.number.name(),
                "http://imeji.org/terms/number", indexes.get(SearchIndex.names.md.name())));
        indexes = put(indexes, new SearchIndex(SearchIndex.names.date.name(),
                "http://imeji.org/terms/date", indexes.get(SearchIndex.names.md.name())));
        indexes = put(indexes, new SearchIndex(SearchIndex.names.time.name(),
                "http://imeji.org/terms/time", indexes.get(SearchIndex.names.md.name())));
        indexes = put(indexes, new SearchIndex(SearchIndex.names.title.name(),
                "http://purl.org/dc/terms/title", indexes.get(SearchIndex.names.md.name())));
        indexes = put(indexes, new SearchIndex(SearchIndex.names.longitude.name(),
                "http://imeji.org/terms/longitude", indexes.get(SearchIndex.names.md.name())));
        indexes = put(indexes, new SearchIndex(SearchIndex.names.latitude.name(),
                "http://imeji.org/terms/latitude", indexes.get(SearchIndex.names.md.name())));
        indexes = put(indexes, new SearchIndex(SearchIndex.names.license.name(),
                "http://imeji.org/terms/license", indexes.get(SearchIndex.names.md.name())));
        indexes = put(indexes, new SearchIndex(SearchIndex.names.link.name(),
                "http://imeji.org/terms/uri", indexes.get(SearchIndex.names.md.name())));
        ...
        indexes = put(indexes, new SearchIndex(SearchIndex.names.person.name(),
                "http://xmlns.com/foaf/0.1/person", indexes.get(SearchIndex.names.md.name())));
        indexes = put(
                indexes,
                new SearchIndex(SearchIndex.names.person_family.name(),
                        "http://purl.org/escidoc/metadata/terms/0.1/family-name", indexes
                                .get(SearchIndex.names.person.name())));
        indexes = put(
                indexes,
                new SearchIndex(SearchIndex.names.person_given.name(),
                        "http://purl.org/escidoc/metadata/terms/0.1/given-name", indexes
                                .get(SearchIndex.names.person.name())));
        indexes = put(
                indexes,
                new SearchIndex(SearchIndex.names.person_id.name(),
                        "http://purl.org/dc/elements/1.1/identifier", indexes
                                .get(SearchIndex.names.person.name())));
        indexes = put(
                indexes,
                new SearchIndex(SearchIndex.names.person_role.name(),
        ...
        indexes = put(
                indexes,
                new SearchIndex(SearchIndex.names.person_org_title.name(),
                        "http://purl.org/dc/terms/title", indexes
                                .get(SearchIndex.names.person_org.name())));
        indexes = put(
                indexes,
                new SearchIndex(SearchIndex.names.person_org_id.name(),
                        "http://purl.org/dc/terms/identifier", indexes
                                .get(SearchIndex.names.person_org.name())));
        indexes = put(
                indexes,
                new SearchIndex(SearchIndex.names.person_org_description.name(),
                        "http://purl.org/dc/terms/description", indexes
                                .get(SearchIndex.names.person_org.name())));
        indexes = put(
                indexes,
                new SearchIndex(SearchIndex.names.person_org_city.name(),
                        "http://purl.org/escidoc/metadata/terms/0.1/city", indexes
                                .get(SearchIndex.names.person_org.name())));
        indexes = put(
                indexes,
                new SearchIndex(SearchIndex.names.person_org_country.name(),
