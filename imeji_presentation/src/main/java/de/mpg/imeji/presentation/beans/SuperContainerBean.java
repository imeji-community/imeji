...
public abstract class SuperContainerBean<T> extends BasePaginatorListSessionBean<T> implements Serializable
{
    ...
    protected void initMenus()
    {
        ...
        sortMenu.add(new SelectItem(SearchIndexes.PROPERTIES_STATUS, sb.getLabel(SearchIndexes.PROPERTIES_STATUS.name())));
        sortMenu.add(new SelectItem(SearchIndexes.CONTAINER_METADATA_TITLE, sb.getLabel(SearchIndexes.CONTAINER_METADATA_TITLE.name())));
        sortMenu.add(new SelectItem(SearchIndexes.PROPERTIES_LAST_MODIFICATION_DATE,sb.getLabel(SearchIndexes.PROPERTIES_LAST_MODIFICATION_DATE.name())));
    ...
    public SearchCriterion getFilter()
    {
    	...
    	{
    		sc = new SearchCriterion(Operator.AND, SearchIndexes.MY_IMAGES, ObjectHelper.getURI(User.class, sb.getUser().getEmail()).toString(), Filtertype.URI);
    	}
    	else if("private".equals(selectedFilter))
    	{
    		sc = new SearchCriterion(Operator.AND, SearchIndexes.PROPERTIES_STATUS, "http://imeji.org/terms/status/PENDING", Filtertype.URI);
    	}
    	else if("public".equals(selectedFilter))
    	{
    		sc = new SearchCriterion(Operator.AND, SearchIndexes.PROPERTIES_STATUS, "http://imeji.org/terms/status/RELEASED", Filtertype.URI);
    	}
    	else if("withdrawn".equals(selectedFilter))
    	{
    		sc = new SearchCriterion(Operator.AND, SearchIndexes.PROPERTIES_STATUS, "http://imeji.org/terms/status/WITHDRAWN", Filtertype.URI);
    	}
