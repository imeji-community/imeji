...
public class PageURIHelper 
{
	public static URI getPageURI(ImejiPages pageType, String q, String[] id) throws IOException, URISyntaxException
	{
		...
		case IMAGE : return URI.create(baseURL + "item/" + id[0] + "/view?h=");
		case COLLECTION_IMAGE : if (id.length == 2) return URI.create(baseURL + "collection/" + id[0] + "/item/" + id[1] + "?h=");
		...
		case ALBUM_IMAGE : if (id.length == 2) return URI.create(baseURL + "album/" + id[0] + "/item/" + id[1] + "?h=");
