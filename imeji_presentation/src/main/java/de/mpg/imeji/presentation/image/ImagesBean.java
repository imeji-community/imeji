...
public class ImagesBean extends BasePaginatorListSessionBean<ThumbnailBean> implements Serializable
{
	...
	private void initMenus()
	{
		...
		sortMenu.add(new SelectItem(SearchIndexes.PROPERTIES_CREATION_DATE, sb.getLabel(SearchIndexes.PROPERTIES_CREATION_DATE.name())));
		sortMenu.add(new SelectItem(SearchIndexes.IMAGE_COLLECTION, sb.getLabel(SearchIndexes.IMAGE_COLLECTION.name())));
		...
		selectedSortCriterion = SearchIndexes.PROPERTIES_CREATION_DATE.name();
	...
	public String deleteAll() throws Exception 
	{
		...
		if (items != null && !items.isEmpty()) coll = cc.retrieve(items.iterator().next().getCollection());
		int count = 0;
		for(Item im : items)
		{
			try 
