...
public class Locks {
	...
	public static boolean isLocked(String uri, String email) {
		...
		if (syslock != null) {
			...
			if (userLock != null && email != null
					&& !userLock.getEmail().equals(email)) {
				return true;
			}
			return false;
		}
	...
	public static void lock(Lock lock) {
		if (!isLocked(lock.getUri(), lock.getEmail())) {
			...
			throw new RuntimeException(lock.getUri()
					+ " already locked by another user " + lock.getEmail());
		}
	}
	...
	public static void unLock(Lock lock) {
		if (lock.getEmail() == null
				&& getSystemLocks().get(lock.getUri()) != null) {
			logger.debug(lock.getUri() + " unlocked by system");
			getSystemLocks().remove(lock.getUri());
		} else if (lock.getEmail() != null
				&& getUserLocks().get(lock.getUri()) != null) {
