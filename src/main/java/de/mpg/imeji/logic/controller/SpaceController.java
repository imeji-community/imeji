...
public class SpaceController extends ImejiController {
	...
	public Space create(Space space, Collection<String> newSpaceCollections,
			File file, User user) throws ImejiException, IOException {
	...
	public Space update(Space space, Collection<String> newSpaceCollections,
			File file, User user) throws ImejiException, IOException {
	...
	/**
	 * Updates a space -Logged in users: --User is space owner --OR user is
	 * space editor
	 *
	 * @param space
	 * @param user
	 ...
	 * @throws ImejiException
	 */
	public Space updateFile(Space space, File f, User user)
			throws ImejiException, IOException {

		space.setLogoUrl(URI.create(generateUrl(
				ObjectHelper.getId(space.getId()), f.getName())));
	...
	public String transformUrlToPath(String url) {
		return URI.create(url).getPath()
				.replace(URI.create(storageUrl).getPath(), storagePath)
	...
	/**
	 * Updates an space -Logged in users: --User is space owner --OR user is
	 * space editor
	 *
	 * @param space
	 * @param user
	...
	public Space retrieve(URI spaceId, User user) throws ImejiException {
		Space space = (Space) reader
				.read(spaceId.toString(), user, new Space());
	...
	public List<Space> retrieveAll() throws ImejiException {
		return Lists.transform(ImejiSPARQL.exec(SPARQLQueries.selectSpaceAll(),
				Imeji.spaceModel), new Function<String, Space>() {
			@Override
	 ...
	 */
	public Collection<String> retrieveCollections(Space space, boolean force)
			throws ImejiException {
		...
		if (force || Iterables.isEmpty(space.getSpaceCollections())) {
			for (String colUri : ImejiSPARQL
					.exec(SPARQLQueries.selectCollectionsOfSpace(space.getId()),
							null)) {
	...
	public Collection<String> retrieveCollections(Space space)
			throws ImejiException {
	 ...
	 * @throws ImejiException
	 */
	public void addCollection(Space space, String collId, User user)
			throws ImejiException {
		if (!Iterables.contains(space.getSpaceCollections(), collId)) {
			
			space.getSpaceCollections().add(collId);
			//update(space, user);
			setSpaceInCollections(space, Lists.newArrayList(collId), user,
					false);
		}
	}
	 ...
	 * @throws ImejiException
	 */
	public void addCollections(Space space, List<String> toAdd, User user)
			throws ImejiException {
	 ...
	 * @throws ImejiException
	 */
	public void addCollection(String spaceId, String collId, User user)
			throws ImejiException {
	 ...
	 * @throws ImejiException
	 */
	public Collection<String> removeCollection(Space space,
			final String collId, User user) throws ImejiException {
	 ...
	 * @throws ImejiException
	 */
	public Collection<String> removeCollections(Space space,
			Collection<String> collsToRemove, User user) throws ImejiException  {
	...
	private void setSpaceInCollections(Space space, Collection<String> collIds,
			User user, boolean remove) throws ImejiException {
		for (String collId : collIds) {
			...
			try {
				...
				throw new UnprocessableError("Error during patching collection "+c.getId()+" with space Id "+c.getSpace());
			}
		}
	}
	...
	public Space retrieveSpaceByLabel(String spaceId, User user)
			throws ImejiException {
		Search s = SearchFactory.create();
		List<String> r = s.searchSimpleForQuery(
				SPARQLQueries.getSpaceByLabel(spaceId)).getResults();
	 ...
	 * @throws ImejiException
	 */
	public void delete(Space space, User user) throws ImejiException,
			IOException {
		removeFile(space);
