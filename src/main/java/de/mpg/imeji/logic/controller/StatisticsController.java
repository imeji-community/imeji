...
public class StatisticsController extends ImejiController {
	...
	public List<String> getAllInstitute() {
		...
		return s.searchSimpleForQuery(SPARQLQueries.selectAllInstitutes())
				.getResults();
	}

	/**
	 * Return the size of the storage used by one institute (i.e. the size of
	 * all files in collections belonging the institut)
	 * 
	 * @param instituteName
	 * @return
	 */
	public long getUsedStorageSizeForInstitute(String instituteName) {
		...
		List<String> result = s.searchSimpleForQuery(
				SPARQLQueries.selectInstituteFileSize(instituteName))
				.getResults();
		if (result.size() == 1 && result.get(0) != null) {
			String size = result.get(0).replace(
