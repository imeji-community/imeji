                        ...
                        .getPropertyDescriptors())
                {
                    ...
                    if (!notRequiredList.contains(propertyDescriptor.getName()))
                    {
                        if (metadata.getValueFromMethod(propertyDescriptor.getReadMethod().getName()) == null
                                || metadata.getValueFromMethod(propertyDescriptor.getReadMethod().getName()).toString()
                                        .isEmpty())
                        {
            ...
            case CONE_PERSON:
                        ...
                        .getPropertyDescriptors())
                {
                    ...
                    if (!notRequiredList.contains(propertyDescriptor.getName()))
                    {
                        if (metadata.getValueFromMethod(propertyDescriptor.getReadMethod().getName()) == null)
                        {
                            throw new Exception(new Throwable("Cone Person object of Item (" + item.getId().toString()
                                    + ") has invalid setting for element: " + propertyDescriptor.getName()));
                        }
                    }
                }
            ...
            case GEOLOCATION:
                        ...
                        .getPropertyDescriptors())
                {
                    ...
                    if (!notRequiredList.contains(propertyDescriptor.getName()))
                    {
                        if (metadata.getValueFromMethod(propertyDescriptor.getReadMethod().getName()) == null)
                        {
                            throw new Exception(new Throwable("Geolocation object of Item (" + item.getId().toString()
                                    + ") has invalid setting for element: " + propertyDescriptor.getName()));
                        }
                    }
                }
            ...
            case PUBLICATION:
                        ...
                        .getPropertyDescriptors())
                {
                    ...
                    if (!notRequiredList.contains(propertyDescriptor.getName()))
                    {
                        if (metadata.getValueFromMethod(propertyDescriptor.getReadMethod().getName()) == null)
                        {
                            throw new Exception(new Throwable("Publication object of Item (" + item.getId().toString()
                                    + ") has invalid setting for element: " + propertyDescriptor.getName()));
                        }
                    }
                }
    ...
    public static void validate(Collection<Metadata> metadata, Item item) throws Exception
    {
        for (Metadata md : metadata)
        {
            MetadataContentValidator
                    .validate(md, MetadataTypesHelper.getTypesForNamespace(md.getTypeNamespace()), item);
        }
    }
}
