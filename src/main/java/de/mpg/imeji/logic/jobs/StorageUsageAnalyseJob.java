...
public class StorageUsageAnalyseJob implements Callable<Integer> {
	private static Logger logger = Logger
			.getLogger(StorageUsageAnalyseJob.class);
	...
	public StorageUsageAnalyseJob() throws IOException, URISyntaxException {
		STORAGE_USAGE_STATS_FILE_NAME = new File(
				PropertyReader.getProperty("imeji.tdb.path")
						+ "/storageStats.xml");
		if (!STORAGE_USAGE_STATS_FILE_NAME.exists())
			STORAGE_USAGE_STATS_FILE_NAME.createNewFile();
		try {
			storageProperties.loadFromXML(new FileInputStream(
					STORAGE_USAGE_STATS_FILE_NAME));
		} catch (Exception e) {
			logger.info("Can not read " + STORAGE_USAGE_STATS_FILE_NAME
					+ " (Probably empty)");
		}
	}
	...
	public Integer call() throws Exception {
		...
		storageProperties.setProperty(StorageUsage.LAST_UPDATE_DATE.name(),
				getCurrentDate());
		storageProperties.storeToXML(new FileOutputStream(
				STORAGE_USAGE_STATS_FILE_NAME), null, "UTF-8");
		logger.info("...done");
		return 1;
	}
	...
	public int getNumberOfFiles() {
		if (storageProperties.get(StorageUsage.NUMBER_OF_FILES.name()) == null)
			return 0;
		return Integer.parseInt((String) storageProperties
				.get(StorageUsage.NUMBER_OF_FILES.name()));
	}

	public long getStorageUsed() {
		if (storageProperties.get(StorageUsage.STORAGE_USED.name()) == null)
			return 0;
		return Long.parseLong((String) storageProperties
				.get(StorageUsage.STORAGE_USED.name()));
	}
	...
	public long getFreeSpace() {
		if (storageProperties.get(StorageUsage.FREE_SPACE.name()) == null)
			return 0;
		return Long.parseLong((String) storageProperties
				.get(StorageUsage.FREE_SPACE.name()));
	}

	public String getLastUpdate() {
		if (storageProperties.get(StorageUsage.LAST_UPDATE_DATE.name()) == null)
			return "-";
		return (String) storageProperties.get(StorageUsage.LAST_UPDATE_DATE
				.name());
	}
}
