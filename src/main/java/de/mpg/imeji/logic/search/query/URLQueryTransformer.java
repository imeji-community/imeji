...
public class URLQueryTransformer {
	...
	/**
	 * Parse a url search query into a {@link SearchQuery}. Decode the query
	 * with UTF-8
	 * 
	 * @param query
	 * @return
	...
	/**
	 * Parse a url search query into a {@link SearchQuery}. The query should be
	 * already decoded
	 * 
	 * @param query
	 * @return
	 * @throws IOException
	 */
	public static SearchQuery parseStringQueryDecoded(String query)
			throws IOException {
		...
		while ((c = reader.read()) != -1) {
			...
			if (scString.trim().equals("AND") || scString.trim().equals("OR")) {
				searchQuery.getElements().add(
						new SearchLogicalRelation(LOGICAL_RELATIONS
			...
			if (hasBracket && (bracketsOpened - bracketsClosed == 0)) {
				...
				if (!subSearchQuery.isEmpty()) {
					...
					searchGroup.getGroup().addAll(
							parseStringQueryDecoded(subQuery).getElements());
					searchQuery.getElements().add(searchGroup);
					subQuery = "";
				}
			}
			if (matchSearchMetadataPattern(scString)) {
				String op = parseOperatorInSearchPattern(scString,
						SEARCH_METADATA_PATTERN);
				int indexOp = scString.indexOf(op);
				int indexValue = scString.indexOf("\"");
				int indexIndex = scString.indexOf(":");
				String value = scString.substring(indexValue + 1,
						scString.length() - 1).trim();
				if (value.startsWith("\"")) {
					value += "\"";
				}
				SearchIndex index = SPARQLSearch.getIndex(scString.substring(
						indexIndex + 1, indexOp).trim());
				SearchOperators operator = stringOperator2SearchOperator(op);
				searchQuery.addPair(new SearchMetadata(index, operator, value,
						ObjectHelper.getURI(Statement.class, scString
								.substring(0, indexIndex).trim()), not));
				not = false;
				scString = "";
				...
				SearchIndex index = SPARQLSearch.getIndex(scString.substring(0,
						indexOp).trim());
				SearchOperators operator = stringOperator2SearchOperator(op);
				searchQuery
						.addPair(new SearchPair(index, operator, value, not));
				scString = "";
				not = false;
			}
		}
	 ...
	 * @return
	 */
	private static String parseOperatorInSearchPattern(String str,
			String pattern) {
	...
	public static String transform2UTF8URL(SearchQuery searchQuery) {
		try {
			...
			throw new RuntimeException("Error encoding search query: "
					+ searchQuery, e);
		}
	}
	...
	public static String transform2URL(SearchQuery searchQuery) {
		...
		for (SearchElement se : searchQuery.getElements()) {
			...
			case GROUP:
				...
				String g = transform2URL(new SearchQuery(
						((SearchGroup) se).getGroup()));
				if (!"".equals(g))
					query += logical + "(" + g + ")";
				break;
			...
			case METADATA:
				...
				query += logical
						+ transformStatementToIndex(
								((SearchMetadata) se).getStatement(),
	 ...
	 * @return
	 */
	public static String transformStatementToIndex(URI statement,
			SearchIndex index) {
		return ObjectHelper.getId(statement) + ":" + index.getName();
	}

	/**
	 * REturn the search value of the {@link SearchMetadata} as string for an
	 * url
	 * 
	 * @param md
	 * @return
	...
	private static String searchPair2PrettyQuery(SearchPair pair) {
		...
		if (pair.getIndex().getName().equals(SearchIndex.names.all.name())) {
			...
			return indexNamespace2PrettyQuery(pair.getIndex().getNamespace())
					+ " " + negation2PrettyQuery(pair.isNot())
					+ searchOperator2PrettyQuery(pair.getOperator()) + " "
					+ pair.getValue();
		}
	}
	...
	/**
	 * Check if the search group is an group with pair about the same metadata.
	 * For instance, when searching for person, the search group will be
	 * conposed of many pairs (family-name, givennane, etc) which sould be
	 * displayed as a pretty query of only one metadata (person = value)
	 * 
	 * @param group
	 * @return
	 */
	 ...
	 * @return
	 */
	private static String searchLogicalRelation2PrettyQuery(
			SearchLogicalRelation rel) {
	...
	/**
	 * Remove a logical operation if is not followed by a non empty search
	 * element
	 * 
	 * @param q
	 * @return
	...
	private static String searchMetadata2PrettyQuery(SearchMetadata md) {
		String label = ((MetadataLabels) BeanHelper
				.getSessionBean(MetadataLabels.class))
				.getInternationalizedLabels().get(md.getStatement());
		if (label == null) {
			label = "Metadata-"
					+ indexNamespace2PrettyQuery(md.getStatement().toString());
		}
		...
		case "url":
			label += "("
					+ ((SessionBean) BeanHelper
							.getSessionBean(SessionBean.class)).getLabel("url")
					+ ")";
			break;
		default:
			break;
		...
		return label + " " + negation2PrettyQuery(md.isNot())
				+ searchOperator2PrettyQuery(md.getOperator()) + " "
				+ md.getValue();
	}
}
