...
public class EscidocStorage implements Storage {
  ...
  /**
   * How a long a login in escidoc is considered as valid. Time is define in ms. The time is set to
   * 1 hour
   */
  ...
  public UploadResult upload(String filename, File file, String collectionId) {
    try {
      // Construct the Item
      item =
          util.itemFactory(PropertyReader.getProperty("escidoc.imeji.content-model.id"),
      ...
      UploadResult result =
          new UploadResult(item.getObjid(), EscidocUtils.getOriginalResolution(item),
   ...
   * @throws Exception
   */
  private void uploadFileAndAdditToItem(File file, FileResolution resolution, String filename)
    ...
    URL url =
        uploadViaStagingArea(imageGeneratorManager.generate(file,
            FilenameUtils.getExtension(filename), resolution));
