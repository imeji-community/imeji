...
public class ImageUtils {
	...
	public static byte[] resizeJPEG(byte[] bytes, FileResolution resolution)
		...
		if (!FileResolution.ORIGINAL.equals(resolution)) {
			...
			bytes = toBytes(scaleImage(image, resolution),
					StorageUtils.getMimeType("jpg"));
		}
		return bytes;
	}
	 ...
	 * resolution images. If the format of the image is not supported, return
	 * null
	 * 
	 * @param bytes
	 * @param mimeType
	...
	private static BufferedImage scaleImageFast(BufferedImage image, int size,
		...
		if (width > height) {
			if (FileResolution.THUMBNAIL.equals(resolution)) {
				...
				rescaledImage = getScaledInstance(image, size, height * size
						/ width, RenderingHints.VALUE_INTERPOLATION_BILINEAR,
						RESCALE_HIGH_QUALITY);
		} else {
			if (FileResolution.THUMBNAIL.equals(resolution)) {
				...
				rescaledImage = getScaledInstance(image, width * size / height,
						size, RenderingHints.VALUE_INTERPOLATION_BILINEAR,
						RESCALE_HIGH_QUALITY);
		}
		BufferedImage rescaledBufferedImage = new BufferedImage(
				rescaledImage.getWidth(null), rescaledImage.getHeight(null),
				colorSpace);
	 ...
	 * @throws IOException
	 */
	public static byte[] toBytes(BufferedImage image, String mimeType)
			throws IOException {
	...
	public static BufferedImage scaleImage(BufferedImage image,
		...
		if (image.getWidth() > size || image.getHeight() > size) {
			...
			size = image.getWidth() > image.getHeight() ? image.getWidth()
					: image.getHeight();
			bufferedImage = scaleImageFast(image, size, resolution);
			// bufferedImage = image;
		}
	 ...
	 * @throws URISyntaxException
	 */
	public static String getEscidocThumbnailContentCategory()
			throws IOException, URISyntaxException {
		return PropertyReader
				.getProperty("xsd.metadata.content-category.thumbnail");
	}
	 ...
	 * @throws URISyntaxException
	 */
	public static String getEscidocWebContentCategory() throws IOException,
			URISyntaxException {
		return PropertyReader
				.getProperty("xsd.metadata.content-category.web-resolution");
	}
	 ...
	 * @throws URISyntaxException
	 */
	public static String getEscidocOriginalContentCategory()
			throws IOException, URISyntaxException {
		return PropertyReader
