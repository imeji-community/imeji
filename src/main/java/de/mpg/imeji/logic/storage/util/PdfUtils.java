...
public class PdfUtils {
  ...
  private static int getResolutionDPI() {
    try {
      int resolution =
          Integer.parseInt(PropertyReader.getProperty("imeji.internal.pdf.resolution.dpi"));
      if (resolution > 0)
        return resolution;
    } catch (Exception e) {
   ...
   * @throws IOException
   */
  public static byte[] pdfsToImageBytes(byte[] bytes, FileResolution resolution)
      throws FileNotFoundException, IOException {
    if (resolution == FileResolution.WEB) {
      return PdfUtils.pdfFileToByteAray(new PDFFile(ByteBuffer.wrap(bytes)),
          PdfUtils.getThumbnailPage(), BufferedImage.TYPE_INT_RGB, PdfUtils.DPI_WEB);
    } else if (resolution == FileResolution.THUMBNAIL) {
      return PdfUtils.pdfFileToByteAray(new PDFFile(ByteBuffer.wrap(bytes)),
          PdfUtils.getThumbnailPage(), BufferedImage.TYPE_INT_RGB, PdfUtils.DPI_THUMB);
    }
    return PdfUtils.pdfFileToByteAray(new PDFFile(ByteBuffer.wrap(bytes)),
        PdfUtils.getThumbnailPage(), BufferedImage.TYPE_INT_RGB, PdfUtils.getResolutionDPI());
  }
   ...
   * @throws Exception
   */
  public static byte[] pdfPageToByteAray(PDFPage page, int imageType, int resolution)
      throws Exception {
    ByteArrayOutputStream baos = new ByteArrayOutputStream();
    try {
      ImageIO.write(PdfUtils.convertToImage(page, imageType, resolution),
          PdfUtils.IMAGE_FILE_EXTENSION, baos);
      baos.flush();
