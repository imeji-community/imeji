...
/**
 * {@link ItemsBean} within an {@link Album}: Used to browse {@link Item} of an
 * {@link Album}
 * 
...
public class AlbumItemsBean extends ItemsBean {
	...
	private void removeAllFromAlbum(Album album) throws ImejiException {
		if (sb.getActiveAlbum() != null
				&& album.getId().toString()
						.equals(sb.getActiveAlbum().getId().toString())) {
	...
	private void removeFromAlbum(List<String> uris) {
		try {
			if (sb.getActiveAlbum() != null
				...
				album = (Album) ic.searchAndSetContainerItems(album,
						sb.getUser(), -1, 0);
				AlbumController ac = new AlbumController();
				int deletedCount = ac
						.removeFromAlbum(album, uris, sb.getUser());
	...
	public String withdraw() throws Exception {
		...
		((AlbumBean) BeanHelper.getSessionBean(AlbumBean.class)).getAlbum()
				.setDiscardComment(dc);
