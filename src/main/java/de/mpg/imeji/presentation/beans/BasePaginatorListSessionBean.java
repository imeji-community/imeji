...
public abstract class BasePaginatorListSessionBean<ListElementType> {
	protected static Logger logger = Logger
			.getLogger(BasePaginatorListSessionBean.class);
	/**
	...
	/**
	 * The total number of elements that are in the complete list (without any
	 * limit or offset filters). corresponding BaseListRetrieverRequestBean.
	 */
	...
	public BasePaginatorListSessionBean() {
		...
		try {
			ajaxMode = Boolean.valueOf(PropertyReader
					.getProperty("imeji.pagination.ajaxmodus"));
		} catch (Exception e) {
	...
	public String getUrlParameters() {
		if (FacesContext.getCurrentInstance().getExternalContext()
				.getRequestParameterMap().containsKey("page")) {
			currentPageNumber = Integer.parseInt(FacesContext
					.getCurrentInstance().getExternalContext()
					.getRequestParameterMap().get("page"));
		}
		if (FacesContext.getCurrentInstance().getExternalContext()
				.getRequestParameterMap().containsKey("el")) {
			elementsPerPage = Integer.parseInt(FacesContext
					.getCurrentInstance().getExternalContext()
	 ...
	 * whenever the list has to be updated. It reads out basic parameters and
	 * calls readOutParamters on implementing subclasses. It uses the
	 * BaseListRetrieverRequestBean in order to retrieve the new list and
	...
	// protected abstract List<ListElementType> getPartList(int offset, int
	// limit);
	/**
	 * Returns the total number of elements, without offset and limit filters.
	 * Drawn from BaseRetrieverRequestBean
	 */
	...
	public void setCookieElementPerPage() {
		CookieUtils.updateCookieValue(
				SessionBean.numberOfItemsPerPageCookieName,
				Integer.toString(elementsPerPage));
	}

	/**
	 * WARNING: USE THIS METHOD ONLY FROM UPPER PAGINATOR SELECTION MENU IN
	 * JSPF. For setting the value manually, use setElementsPerPage().
	 * 
	...
	/**
	 * WARNING: USE THIS METHOD ONLY FROM LOWER PAGINATOR SELECTION MENU IN
	 * JSPF. For setting the value manually, use setElementsPerPage().
	 * 
	 * @param elementsPerPageTop
	 */
	...
	/**
	 * WARNING: USE THIS METHOD ONLY FROM UPPER PAGINATOR SELECTION MENU IN
	 * JSPF. For getting the value manually, use getElementsPerPage().
	 * 
	 * @param elementsPerPageTop
	 */
	@Deprecated
	 ...
	 * @throws Exception
	 */
	public void elementsPerPageTopListener(ValueChangeEvent event)
			throws Exception {
		if (event != null) {
			setElementsPerPage((Integer) event.getNewValue());
			// set new PageNumber to a number where the first element of the
	...
	/**
	 * Used as action when the user sends an value from the upper go to input
	 * field
	 * 
	 * @return
	 * @throws Exception
	...
	/**
	 * Used as action when the user sends an value from the lower go to input
	 * field
	 * 
	 * @return
	 * @throws Exception
	...
	/**
	 * Returns a list with the paginator pages. Used from jsf to iterate over
	 * the numbers
	 * 
	 * @return
	 */
	...
	/**
	 * Returns the number of the paginator page button that should be displayed
	 * as first button of the paginator in order to always display exactly seven
	 * paginator page buttons
	 * 
	 * @return
	 */
	public int getFirstPaginatorPageNumber() {
		if (getPaginatorPageSize() > 7
				&& currentPageNumber > getPaginatorPageSize() - 4) {
	 ...
	 */
	public void setElementsPerPageSelectItems(
			List<SelectItem> elementsPerPageSelectItems) {
	...
	/**
	 * Inner class pf which an instance represents an paginator button. Used by
	 * the iterator in jsf.
	 * 
	 * @author Markus Haarlaender (initial creation)
	 * @author $Author: haarlaender $ (last modification)
	 * @version $Revision: 3185 $ $LastChangedDate: 2009-11-20 14:30:15 +0100
	 *          (Fri, 20 Nov 2009) $
	 */
	public class PaginatorPage {
		...
		/**
		 * Returns the link that is used as output link of the paginator page
		 * button
		 * 
		 * @return
		 */
	...
	/**
	 * WARNING: USE THIS METHOD ONLY FROM UPPER GO TO INPUT FIELD MENU IN JSPF.
	 * For setting the value manually, use setGoToPage().
	 * 
	 * @param elementsPerPageTop
	 */
	@Deprecated
	...
	/**
	 * WARNING: USE THIS METHOD ONLY FROM UPPER GO TO INPUT FIELD MENU IN JSPF.
	 * For getting the value manually, use getGoToPage().
	 * 
	 * @param elementsPerPageTop
	 */
	@Deprecated
	...
	/**
	 * WARNING: USE THIS METHOD ONLY FROM LOWER GO TO INPUT FIELD MENU IN JSPF.
	 * For setting the value manually, use setGoToPage().
	 * 
	 * @param elementsPerPageTop
	 */
	 ...
	 * 
	 * @param offset
	 *            An offset from where the list must start (0 means at the
	 *            beginning of all records)
	 * @param limit
	 *            The length of the list that has to be returned. If the whole
	 *            list has less records than this paramter allows, a smaller
	 *            list can be returned.
	 * @param additionalFilters
	 *            Additional filters that have to be included when retrieving
	 *            the list.
	 * @return
	 * @throws Exception
	 */
