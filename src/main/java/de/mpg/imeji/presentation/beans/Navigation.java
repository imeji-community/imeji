...
public class Navigation implements Serializable {
	...
	public Navigation() throws Exception {
		frameworkUrl = PropertyReader
				.getProperty("escidoc.framework_access.framework.url");
		...
		externalDigilibUrl = PropertyReader
	...
	public String getCreateCollectionUrl() {
		return applicationUrl + getSpacePath() + CREATE.getPath()
				+ COLLECTION.getPath();
	}
	...
	public String getSpacePath() {
		if (!(isNullOrEmpty(((SessionBean) BeanHelper
				.getSessionBean(SessionBean.class)).getSpaceId()))) {
			return spaceCommonSlug
					+ StringHelper.normalizeURI(((SessionBean) BeanHelper
							.getSessionBean(SessionBean.class)).getSpaceId());
		}
		return "";
	}
	...
	public String getContext() {
		...
		if ("details".equals(sessionBean.getCurrentPage().name)
				...
				|| "comparisonFromAlbum"
						.equals(sessionBean.getCurrentPage().name)
