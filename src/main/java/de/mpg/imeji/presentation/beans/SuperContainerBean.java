 ...
 * @param <T>
 */
public abstract class SuperContainerBean<T> extends
		BasePaginatorListSessionBean<T> {
	...
	public SuperContainerBean() {
		...
		try {
			String options = PropertyReader
					.getProperty("imeji.container.list.size.options");
			for (String option : options.split(",")) {
				getElementsPerPageSelectItems().add(new SelectItem(option));
			}
		} catch (Exception e) {
			logger.error(
					"Error reading property imeji.container.list.size.options",
					e);
		}
	}
	...
	public void setCookieElementPerPage() {
		CookieUtils.updateCookieValue(
				SessionBean.numberOfContainersPerPageCookieName,
	...
	protected void initMenus() {
		...
		if (sb.getUser() != null) {
			...
			filterMenu.add(new SelectItem("private", sb
					.getLabel("only_private")));
		}
		filterMenu.add(new SelectItem("public", sb.getLabel("only_public")));
		filterMenu.add(new SelectItem("withdrawn", sb
				.getLabel("only_withdrawn")));
	}
	...
	/**
	 * Return the current {@link Filter} for the {@link List} of
	 * {@link Container}
	 * 
	 * @return
	 */
