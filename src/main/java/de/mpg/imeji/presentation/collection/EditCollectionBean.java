...
public class EditCollectionBean extends CollectionBean {
	...
	public void init() throws Exception {
		...
		if (id != null) {
			((ViewCollectionBean) BeanHelper
					.getSessionBean(ViewCollectionBean.class)).setId(id);
			((ViewCollectionBean) BeanHelper
					.getSessionBean(ViewCollectionBean.class)).init();
			setProfile(((ViewCollectionBean) BeanHelper
					.getSessionBean(ViewCollectionBean.class)).getProfile());
			setCollection(((ViewCollectionBean) BeanHelper
					.getSessionBean(ViewCollectionBean.class)).getCollection());
			...
			((CollectionSessionBean) BeanHelper
			...
			FacesContext
					...
					.redirect(
							navigation.getCollectionUrl()
									+ ObjectHelper.getId(getCollection()
											.getId()) + "/"
	...
	public boolean saveEditedCollection() {
		try {
			...
			if (isUseMDProfileTemplate() && getProfileTemplate() != null) {
				ic = collectionController
						.updateLazyWithProfile(
								getCollection(),
								getProfileTemplate(),
								user,
			...
			if (getIngestImage() != null) {
				collectionController.updateCollectionLogo(ic, getIngestImage()
						.getFile(), user);
				setIngestImage(null);
			}
			...
			BeanHelper.error(sessionBean
					.getMessage("error_collection_logo_save"));
			return false;
		} catch (URISyntaxException e) {
			BeanHelper.error(sessionBean
					.getMessage("error_collection_logo_uri_save"));
			return false;
		}
	...
	public String getCancel() {
		Navigation nav = (Navigation) BeanHelper
				.getApplicationBean(Navigation.class);
		return nav.getCollectionUrl()
				+ ObjectHelper.getId(getCollection().getId()) + "/"
	...
	/**
	 * Method for save&editProfile button. Create the {@link MetadataProfile}
	...
	public String saveAndEditProfile() throws Exception {
		if (saveEditedCollection()) {
			FacesContext
					...
					.getExternalContext()
					.redirect(
							navigation.getProfileUrl() + getProfileId()
									+ "/edit?init=1&col="
