...
public class HistoryUtil {
	private static Pattern findItemId = Pattern.compile(ImejiPages.ITEM_DETAIL
			.getRegex());
	private static Pattern findCollectionId = Pattern
			.compile(ImejiPages.COLLECTION_HOME.getRegex());
	private static Pattern findAlbumId = Pattern.compile(ImejiPages.ALBUM_HOME
			.getRegex());
	private static Pattern findUserGroupId = Pattern
			.compile(ImejiPages.USER_GROUP.getRegex());
	private static Pattern findUserId = Pattern.compile(ImejiPages.USER
			.getRegex());
	...
	public static URI extractURI(String url) {
		...
		if (m.find())
			return ObjectHelper
					.getURI(User.class, UrlHelper.decode(m.group(1)));
		m = findUserGroupId.matcher(url);
		if (m.find())
			return ObjectHelper.getURI(UserGroup.class,
					UrlHelper.decode(m.group(1)));
		return null;
	}
