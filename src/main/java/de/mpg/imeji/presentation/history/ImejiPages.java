...
public enum ImejiPages {
	...
	/*
	 * Order is important: The History will follow the regex rules following
	 * this order to find a match
	 */
	ITEM_DETAIL("", ".*/item/(\\w+).*"), COLLECTIONS("history_collections",
			"/collections"), COLLECTION_ITEMS("history_images_collection",
			".*/collection/(\\w+)/browse.*"), COLLECTION_UPLOAD(
			"history_upload", ".*/collection/(\\w+)/upload.*"), COLLECTION_INFOS(
			"history_collection_info", ".*/collection/(\\w+)/infos.*"), COLLECTION_HOME(
			"collection", ".*/collection/(\\w*).*"), ALBUMS("albums", "/albums"), ALBUM_ITEMS(
			"history_images_album", ".*/album/(\\w+)/browse.*"), ALBUM_INFOS(
			"history_album_info", ".*/album/(\\w+)/infos.*"), ALBUM_HOME(
			"history_album", ".*/album/(\\w*).*"), ADMINISTRATION("admin",
			"/admin"), USER("user", ".*/user\\?id=(.*)&?"), USERS(
			"admin_info_users", "/users"), USER_CREATE("admin_user_new",
			"/createuser"), USER_GROUP_CREATE("admin_userGroup_new",
			"/createusergroup"), USER_GROUP("admin_userGroup",
			...
			"history_images", "/browse"), HOME("history_home", "/");
