...
public class SingleItemBrowse {
  ...
  /**
   * Initialize the {@link SingleItemBrowse} for the current {@link Item} and {@link ItemsBean}
   * according to:
   * 
  ...
  public void init(String type, String containerId) {
    ...
    if (type == "collection") {
      ...
      baseUrl =
          ((Navigation) BeanHelper.getApplicationBean(Navigation.class)).getAlbumUrl()
              + containerId + "/item/";
    }
    URI nextImage = getNextImageFromList();
    URI prevImage = getPreviousImageFromList();
    String direction =
        FacesContext.getCurrentInstance().getExternalContext().getRequestParameterMap().get("nav");
  ...
  private URI getLastImageOfList() {
    if (imagesBean.getCurrentPartList().size() > 0) {
      return imagesBean.getCurrentPartList().get(imagesBean.getCurrentPartList().size() - 1)
          .getUri();
    }
    return null;
  }
