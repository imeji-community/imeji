...
public class InternationalizationBean {
	...
	private Locale userLocale = FacesContext.getCurrentInstance()
	...
	/**
	 * Menu with first, the supported languages out of the properties, second
	 * all the iso languages
	 * 
	...
	public void readSupportedLanguagesProperty() {
		try {
			supportedLanguages = PropertyReader.getProperty(
					"imeji.i18n.languages").split(",");
			supportedLanguages = ConfigurationBean.getLanguagesStatic().split(
					",");
		} catch (Exception e) {
			throw new RuntimeException(
					"Error reading property imeji.i18n.languages. Check Propety file: "
							+ e);
		}
	}
	...
	/**
	 * True if a language (defined in iso639_1) is supported in imeji (according
	 * to the properties)
	 * 
	 * @param langString
	 * @return
	...
	/**
	 * Return: <br/>
	 * - the supported languages (i.e languages with a translation for labels
	 * and messages) if the parameter is set to true <br/>
	...
	private void changeLanguage(String languageString) {
		...
		CookieUtils.updateCookieValue(SessionBean.langCookieName, session
	...
	public void currentlanguageListener(ValueChangeEvent event) {
		if (event != null
				&& !event.getNewValue().toString()
						.equals(event.getOldValue().toString())) {
	...
	/**
	 * Method called when the user changed the language. The new language is
	 * setted via the listener. The method reload the current page
	 * 
	 * @return
	 * @throws IOException
	 */
	 ...
	 */
	public void setInternationalizedLanguages(
			List<SelectItem> internationalizedLanguages) {
