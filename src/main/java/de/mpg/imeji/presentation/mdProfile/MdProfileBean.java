...
public class MdProfileBean {
	...
	/**
	 * If a {@link Statement} already used by {@link Metadata} has been removed,
	 * return true;
	 */
	...
	public MdProfileBean() {
		collectionSession = (CollectionSessionBean) BeanHelper
				.getSessionBean(CollectionSessionBean.class);
	...
	public void addFirstStatement() {
		...
		getWrappers().add(
				new StatementWrapper(firstStatement, getProfile().getId(),
						getLevel(firstStatement)));
	}
	...
	public String getTypeLabel(String uri) {
		for (Metadata.Types t : Metadata.Types.values()) {
			if (t.getClazzNamespace().equals(uri))
				return ((SessionBean) BeanHelper
						.getSessionBean(SessionBean.class)).getLabel("facet_"
						+ t.name().toLowerCase());
		}
		return uri;
	}
	...
	public void insertMetadata() {
		...
		StatementWrapper dropped = wrappers
				.get(getStatementPosition() > 0 ? getStatementPosition() - 1
						: 0);
		boolean moved = insertWrapper(dragged, getStatementPosition());
		if (moved) {
			dragged.getStatement()
					.setParent(dropped.getStatement().getParent());
		}
		reInitWrappers();
	}
	...
	/**
	 * Insert a {@link StatementWrapper} into the {@link List} at the position
	 * passed in the parameter. The childs of the wrapper are inserted after it.
	 * 
	 * @param w
	 *            - The wrapper to insert
	 * @param to
	 *            - The position to insert in the list
	 * @return
	 */
	...
	public boolean hasChild(int position) {
		if (position < wrappers.size()
				&& wrappers.get(position).getStatement().getParent() != null)
			return wrappers
					.get(position)
					.getStatement()
					.getParent()
					.compareTo(
							wrappers.get(position - 1).getStatement().getId()) == 0;
		return false;
	}
	 ...
	 * 
	 * @param position
	 *            - The position after to position are incremented
	 * @param toIncrement
	...
	private void incrementPosition(int position, int toIncrement) {
		for (StatementWrapper w : wrappers) {
			if (w.getStatement().getPos() >= position)
				w.getStatement()
						.setPos(w.getStatement().getPos() + toIncrement);
		}
	}
	...
	/**
	 * Reset the position of the {@link Statement} according to the current
	 * order of the {@link StatementWrapper} {@link List}
	 */
	...
	/**
	 * The the parent {@link StatementWrapper} of the dropped
	 * {@link StatementWrapper}. This might change if its parent is the one
	 * being dragged
	 * 
	 * @param dragged
	 * @param dropped
	 * @return
	 */
	private StatementWrapper setParentOfDropped(StatementWrapper dragged,
			StatementWrapper dropped) {
		if (isAParent(dragged, dropped)) {
			StatementWrapper firstChild = findFirstChild(dragged);
			if (firstChild != null)
				firstChild.getStatement().setParent(
						dragged.getStatement().getParent());
		}
		return dropped;
	}
	...
	private boolean isAParent(StatementWrapper parent, StatementWrapper child) {
		while (child.getStatement().getParent() != null) {
			if (child.getStatement().getParent()
					.compareTo(parent.getStatement().getId()) == 0) {
				return true;
			} else if (child.getStatement().getPos() - 1 >= 0) {
				StatementWrapper parentOfChild = wrappers.get(child
						.getStatement().getPos() - 1);
	 ...
	 * @param parent
	 * @param firstOnly
	 *            if true, return only the direct childs
	 * @return
	 */
	private List<StatementWrapper> getChilds(StatementWrapper parent,
		...
		for (StatementWrapper wrapper : wrappers) {
			if (wrapper.getStatement().getParent() != null
					&& wrapper.getStatement().getParent()
							.compareTo(parent.getStatement().getId()) == 0) {
				l.add(wrapper);
				if (!firstOnly)
	 ...
	 */
	public void moveUp() {
		Collections.swap(wrappers, getStatementPosition(),
				getStatementPosition() + 1);
	}
	...
	public void moveDown() {
		Collections.swap(wrappers, getStatementPosition() + 1,
	...
	protected int getLevel(Statement st) {
		if (!levels.containsKey(st.getId())) {
			if (st.getParent() != null && levels.get(st.getParent()) != null) {
				levels.put(
						st.getId(),
						(levels.get(st.getParent()) + MARGIN_PIXELS_FOR_STATEMENT_CHILD));
			} else {
	...
	/**
	 * Find the next {@link Statement} in the {@link Statement} list which have
	 * the same level, which means, the first {@link Statement} which is not a
	 * child
	 * 
	 * @param st
	 * @return
	...
	/**
	 * Called by add statement button. Add a new statement to the profile. The
	 * position of the new statement is defined by the button position
	 */
	public void addStatement() {
		if (wrappers.isEmpty()) {
			wrappers.add(new StatementWrapper(ImejiFactory.newStatement(),
					profile.getId(), 0));
		} else {
			Statement previousStatement = wrappers.get(getStatementPosition())
					.getStatement();
			Statement newStatement = ImejiFactory
					.newStatement(previousStatement.getParent());
			wrappers.add(findNextStatementWithSameLevel(previousStatement),
					new StatementWrapper(newStatement, profile.getId(),
							getLevel(newStatement)));
		}
	}

	/**
	 * Called by remove statement button. If the statement is not used by an
	 * imeji item, remove it, according to the position of the button. If the
	 * statement is used, display a warning message in a panel
	 */
	...
	/**
	 * Called by add statement child button. Add a new statement to the profile
	 * as a child of the previous statement. The position of the new statement
	 * is defined by the button position
	 */
	public void addStatementChild() {
		if (!wrappers.isEmpty()) {
			URI parent = wrappers.get(getStatementPosition()).getStatement()
					.getId();
			Statement newChild = ImejiFactory.newStatement(parent);
			wrappers.add(getStatementPosition() + 1, new StatementWrapper(
					newChild, profile.getId(), getLevel(newChild)));
		}
	}

	/**
	 * Remove a {@link Statement} even if it is used by a an item. All
	 * {@link Metadata} using this {@link Statement} are then removed.
	 */
	...
	/**
	 * Remove a {@link StatementWrapper} and all its childs from the
	...
	public void removeLabel() {
		((List<LocalizedString>) wrappers.get(getStatementPosition())
				.getStatement().getLabels()).remove(getLabelPosition());
	}
	...
	public void removeConstraint() {
		Statement st = wrappers.get(getStatementPosition()).getAsStatement();
		((List<String>) st.getLiteralConstraints())
				.remove(getConstraintPosition());
		collectionSession.setProfile(profile);
	}
	...
	/**
	 * return the {@link List} of {@link StatementWrapper} as a {@link List} of
	 * {@link Statement}
	 * 
	 * @return
	 */
	...
	/**
	 * @param draggedStatementPosition
	 *            the draggedStatementPosition to set
	 */
