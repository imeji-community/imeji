...
public class StatementWrapper implements Comparable<StatementWrapper> {
	...
	private int level = 0;
	/**
	 * True if this {@link Statement} is used by at least on {@link Metadata} in
	 * imeji
	 */
	...
	/**
	 * Initialize the {@link StatementWrapper} fields with {@link Statement}
	 * fields
	 * 
	 * @param st
	 */
	public void init(Statement st) {
		...
		used = ImejiSPARQL
				.exec("PREFIX fn: <http://www.w3.org/2005/xpath-functions#> SELECT DISTINCT ?s WHERE {?s <"
						...
						+ "> ?md . ?md <http://imeji.org/terms/statement> <"
	...
	/**
	 * Initialize the {@link VocabularyHelper} according to the
	 * {@link Statement}
	 */
	private void initVocabulary() {
		...
		if (statement.getVocabulary() != null) {
			...
			if ("unknown".equals(vocabularyHelper.getVocabularyName(statement
					.getVocabulary()))) {
				vocabularyHelper.getVocabularies().add(
						new SelectItem(statement.getVocabulary().toString(),
								vocabularyString));
			}
		} else {
	...
	/**
	 * Return the id of the {@link Statement} (i.e. the last part of the
	 * {@link URI})
	 * 
	 * @return
	 */
	...
	/**
	 * Is called when the user select a statement or a language in the drop down
	 * list
	 */
	public void select() {
		// do nothing
