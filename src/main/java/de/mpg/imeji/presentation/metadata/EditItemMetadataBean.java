...
import de.mpg.imeji.presentation.util.*;
...
public class EditItemMetadataBean {
	...
	/**
	 * the {@link EditorItemBean} used to for the editor and which will be
	 * copied to all {@link Item}
	 */
	...
	/**
	 * Read the url paramameters when the page is first called. This method is
	 * called directly from the xhtml page
	 * 
	 * @return
	 */
	...
	private void initProfileAndStatement(List<Item> items) {
		...
		if (items != null && items.size() > 0) {
			profile = ObjectLoader.loadProfile(items.get(0).getMetadataSet()
					.getProfile(), session.getUser());
		}
	...
	private void initEditor(List<Item> items) {
		try {
			...
			if (statement != null) {
				editor = new MetadataMultipleEditor(items, profile,
	...
	private void initEmtpyEditorItem() {
		...
		emtpyItem.getMetadataSets().add(
				ImejiFactory.newMetadataSet(profile.getId()));
		editorItem = new EditorItemBean(emtpyItem, profile, true);
		editorItem.getMds().addEmtpyValues();
	}

	/**
	 * Init the radio select menu with the 3 edit modes (overwrite all values,
	 * append new value, add if empty)
	 */
	private void initModeMenu() {
		selectedMode = "basic";
		modeRadio = new ArrayList<SelectItem>();
		modeRadio
				.add(new SelectItem("basic", ((SessionBean) BeanHelper
						.getSessionBean(SessionBean.class))
	...
	/**
	 * Initialize the select menu with the possible statement to edit (i.e.
	 * statement of the profiles)
	 */
	...
	/**
	 * Lock the {@link Item} which are currently in the editor. This prevent
	 * other users to make concurrent modification.
	 * 
	 * @param items
	 */
	...
	private void unlockImages() {
		...
		for (EditorItemBean eib : editor.getItems()) {
			Locks.unLock(new Lock(eib.asItem().getId().toString(), sb.getUser()
					.getEmail()));
		}
	}
	...
	/**
	 * fill all emtpy Metadata of passed {@link EditorItemBean} with the values
	 * of the current one
	 * 
	 * @param im
	 * @return
	...
	private EditorItemBean pasteMetadataIfEmtpy(EditorItemBean eib) {
		List<SuperMetadataBean> list = fillEmtpyValues(eib.getMds().getTree()
				.getList(), editorItem.getMds().getTree().getList());
	...
	/**
	 * Add an emtpy {@link Metadata} accroding to the current {@link Statement}
	 * to the {@link EditorItemBean}. If the {@link EditorItemBean} has already
	 * an emtpy {@link Metadata} for this {@link Statement}, then don't had it.
	 * 
	 * @param eib
	 */
	...
	public boolean isEditableStatement(Statement st) {
		...
		return statement.getId().compareTo(st.getId()) == 0
				|| (lastParent != null && statement.getId().compareTo(
						lastParent) == 0);
	}
	...
	/**
	 * @param editorItemBean
	 *            the editorItemBean to set
	 */
