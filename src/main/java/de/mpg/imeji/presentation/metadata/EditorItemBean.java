...
public class EditorItemBean {
	...
	public EditorItemBean(Item item, MetadataProfile profile,
		...
		this.item = item;
		mds = new MetadataSetBean(this.item.getMetadataSet(), this.profile,
				addEmtpyValue);
	}
	...
	/**
	 * Clear the {@link Metadata} for one {@link Statement}: remove all
	 * {@link Metadata} and its Childs and add an empty one
	 * 
	 * @param st
	 */
	public void clear(Statement st) {
		for (SuperMetadataBean smd : mds.getTree().getList()) {
			if (st.getId().compareTo(smd.getStatement().getId()) == 0) {
				// Clear the childs
				for (SuperMetadataBean child : mds.getTree().getChilds(
						smd.getTreeIndex()))
					child.clear();
				// clear the metadata
				smd.clear();
	...
	/**
	 * Return the position of the last {@link SuperMetadataBean} in the editor
	 * for this {@link Statement}
	 * 
	 * @param st
	 * @return
	 */
