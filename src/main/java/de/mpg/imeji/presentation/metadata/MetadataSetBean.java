...
public class MetadataSetBean implements Serializable {
	...
	private static final Logger logger = Logger
			.getLogger(MetadataSetBean.class);
	 ...
	 * @param mds
	 * @param addEmtpyValue
	 *            if true, add an emtpy metadata for all {@link Statement} which
	 *            don't have any value
	 */
	public MetadataSetBean(MetadataSet mds, MetadataProfile profile,
	...
	/**
	 * Initialize the {@link MetadataSetBean} with a flat {@link List} of
	 * {@link SuperMetadataBean}
	 * 
	 * @param flat
	 */
	...
	/**
	 * Remove all Emtpy {@link SuperMetadataBean} of the {@link MetadataSetBean}
	 * (if and only if the {@link SuperMetadataBean} doesn't have a child)
	 */
	public void trim() {
	...
	public void appendEmtpyMetadata(Statement st) {
		metadataTree.add(new SuperMetadataBean(MetadataFactory
				.createMetadata(st), st));
	}

	/**
	 * Create a {@link List} of {@link SuperMetadataBean} from a list of
	 * {@link Metadata}
	 * 
	 * @return
	 */
	private List<SuperMetadataBean> toSuperList(List<Metadata> l) {
		...
		for (Metadata md : l) {
			Statement st = ProfileHelper.getStatement(md.getStatement(),
					profile);
			if (st != null) {
				SuperMetadataBean smd = new SuperMetadataBean(md, st);
				flat.add(smd);
			} else {
				logger.error("A metadata "
						+ md.getId()
						+ " is defined with a non existing statement in profile "
						+ this.profile.getId());
			}
		}
		return flat;
	...
	/**
	 * Add to the {@link SuperMetadataTree} new emtpy {@link SuperMetadataBean}
	 * when it hadn't been already defined one. this method does the contrary as
	 * the trim() method
	 */
	public void addEmtpyValues() {
		...
		List<SuperMetadataBean> l = new ArrayList<SuperMetadataBean>(
				metadataTree.getList());
		for (Statement st : profile.getStatements()) {
			if (!exists(st))
				l.add(new SuperMetadataBean(MetadataFactory.createMetadata(st),
						st));
		}
		// Reinit the tree
		initTreeFromList(l);
	...
	/**
	 * True if the {@link MetadataSetBean} has one {@link Metadata} with this
	 * {@link Statement}
	 * 
	 * @param st
	 * @return
	...
	/**
	 * True if the {@link MetadataSetBean} has one {@link Metadata} with this
	 * {@link Statement} which is not emtpy
	 * 
	 * @param st
	 * @return
	 */
	public boolean existsNotEmtpy(Statement st) {
		for (SuperMetadataBean md : metadataTree.getList()) {
			if (!MetadataHelper.isEmpty(md.getMetadata()))
				if (md.getStatement().getId().compareTo(st.getId()) == 0
						|| ProfileHelper.isParent(st, md.getStatement(),
								profile))
					return true;
		}
		return false;
