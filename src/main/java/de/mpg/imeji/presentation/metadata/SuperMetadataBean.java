...
/**
 * Bean for all Metadata types. This bean should have all variable that have
 * been defined in all metadata types.
 * 
 * @author saquet
 */
public class SuperMetadataBean implements Comparable<SuperMetadataBean> {
	...
	private int pos = 0;
	/**
	 * The parent {@link SuperMetadataBean} (i.e {@link Metadata}), according to
	 * what is defined in the {@link MetadataProfile}
	 */
	...
	/**
	 * Return the {@link Metadata} which has been used to initialize this
	 * {@link SuperMetadataBean} Not to use to save the
	 * {@link SuperMetadataBean} as a {@link Metadata} in the database. In this
	...
	/**
	 * Change the Id of the {@link Metadata} which will force the create a new
	 * {@link Metadata} resource in the database
	 * 
	...
	public SuperMetadataBean copy() {
		...
		SuperMetadataBean copy = new SuperMetadataBean(
				MetadataFactory.copyMetadata(asMetadata()), statement);
	...
	public SuperMetadataBean copyEmpty() {
		metadata.setId(IdentifierUtil.newURI(Metadata.class));
		SuperMetadataBean copy = new SuperMetadataBean(
				MetadataFactory.createMetadata(statement), statement);
		copy.setParent(parent);
		copy.setTreeIndex(treeIndex);
		return copy;
	}
	...
	/**
	 * Parse the input for a pattern like: uri:http://url.com, and set the value
	 * (in this case the uri value)
	 * 
	 * @param s
	 */
	private void patternParser(String s) {
		...
		toNull = uriString != null || nameString != null || longString != null
				|| latString != null;
		if (!toNull) {
			name = s;
			label = s;
	...
	/**
	 * Retun the id (last part of the {@link URI}) of the {@link Statement}.
	 * Used for GUI representation
	 * 
	 * @return
	 */
	...
	public void addEmtpyChilds(MetadataProfile p) {
		for (Statement st : p.getStatements()) {
			if (st.getParent() != null
					&& st.getParent().compareTo(statement.getId()) == 0) {
				SuperMetadataBean child = new SuperMetadataBean(
						MetadataFactory.createMetadata(st), st);
	...
	public String addOrganization(int organizationPosition) {
		List<Organization> orgs = (List<Organization>) this.person
				.getOrganizations();
