...
public class SuperMetadataTree implements Serializable {
	 ...
	 * @return
	 */
	private Map<String, SuperMetadataBean> createMap(
			List<SuperMetadataBean> list, SuperMetadataBean parent, String index) {
		...
		for (SuperMetadataBean smd : list) {
			if ((parent == null && isRootMetadata(smd)) || isChild(smd, parent)) {
				...
				map.putAll(createMap(removeFromList(list, smd), smd,
						addIndex(index, "0")));
	...
	private boolean isChild(SuperMetadataBean smb, SuperMetadataBean parent) {
		if (parent == null || smb.getParent() == null)
			...
			return smb.getParent().getMetadata().getId()
					.compareTo(parent.getMetadata().getId()) == 0;
	}

	/**
	 * Remove the {@link SuperMetadataBean} smb from the {@link List}. A new
	 * list is returned, which allows to use this method within the for loop on
	 * the list elements
	 * 
	 * @param list
	 * @param smb
	 * @return
	 */
	private List<SuperMetadataBean> removeFromList(
			List<SuperMetadataBean> list, SuperMetadataBean smb) {
	...
	public List<SuperMetadataBean> getList() {
		List<SuperMetadataBean> list = new ArrayList<SuperMetadataBean>(
				map.values());
	...
	/**
	 * Add a {@link SuperMetadataBean} to the {@link SuperMetadataTree} at the
	 * next position
	 * 
	 * @param smd
	 */
	...
	/**
	 * Insert a {@link SuperMetadataBean} and all its childs in the
	 * {@link SuperMetadataTree} at the position defined in the getIndexTree()
	 * method. If the position is alreay used by a {@link SuperMetadataBean},
	...
	/**
	 * Move a {@link SuperMetadataBean} and all its childs to the next position
	 * in the tree
	 * 
	 * @param smd
	 */
	...
	/**
	 * True if the index1 is a parent of the index2 (not necessary the direct
	 * parent):<br/>
	...
	private boolean isParent(String index1, String index2) {
		return index2.startsWith(index1 + ",")
	...
	/**
	 * Reset the position to all Metadata according to the current order. The
	 * list should be flat
	 */
	public static List<SuperMetadataBean> resetPosition(
			List<SuperMetadataBean> l) {
		int i = 0;
	...
	/**
	 * {@link Comparator} to sort the {@link SuperMetadataBean} in the
	 * {@link SuperMetadataTree}
	 * 
	 * @author saquet (initial creation)
	 * @author $Author$ (last modification)
	 * @version $Revision$ $LastChangedDate$
	...
	private class TreeComparator implements Comparator<SuperMetadataBean> {
		...
		public int compare(SuperMetadataBean md1, SuperMetadataBean md2) {
			...
			int minLength = (index1.length < index2.length ? index1.length
					: index2.length);
	...
	/**
	 * Set the Parent {@link SuperMetadataBean} to all element of the list. This
	 * is made according to the current order of the list (see findBestParent()
	 * method)
	 * 
	 * @param flat
	 * @return
	 ...
	 * {@link SuperMetadataBean}. This is Dependent to the statement (if it has
	 * a parent statement), and the position of the child in the list
	 * 
	 * @param md
	 * @param list
	 * @return
	 */
	private SuperMetadataBean findBestParent(SuperMetadataBean child,
		...
		for (SuperMetadataBean md : list) {
			...
			if (child.getStatement().getParent()
					.compareTo(md.getStatement().getId()) == 0)
				candidates.add(md);
			if (child.getStatement().getId()
					.compareTo(md.getStatement().getId()) == 0
					&& !found)
				position++;
		}
