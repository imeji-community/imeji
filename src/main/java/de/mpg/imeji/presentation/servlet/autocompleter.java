...
public class autocompleter extends HttpServlet {
	...
	private Pattern conePattern = Pattern.compile(
			"http.*/cone/.*?format=json.*", Pattern.CASE_INSENSITIVE);
	private Pattern coneAuthorPattern = Pattern.compile(
			"http.*/cone/persons/.*?format=json.*", Pattern.CASE_INSENSITIVE);
		...
		if (suggest == null || suggest.isEmpty()) {
			...
			if ("imeji_persons".equals(datasource)) {
				...
				try {
					...
					responseString = new String(StorageUtils.toBytes(getMethod
	...
	private String appendResponseForInternalSuggestion(String response,
	...
	/**
	 * parse JSON string returned from remote source by JSON-simple add
	 * properties [ { label: "Choice1", value: "value1" }, ... ] to fit JQuery
	 * UI auto-complete pop format
	 * 
	 * @param input
	 * @param source
	 * @return
	...
	private String parseGoogleGeoAPI(String google) throws IOException {
		...
		for (int i = 0; i < array.size(); ++i) {
			...
			JSONObject location = (JSONObject) ((JSONObject) parseObject
	...
	/**
	 * Parse a JSON file from CoNE with authors, and return a JSON which can be
	 * read by imeji autocomplete
	 * 
	 * @param cone
	 * @return
	 * @throws IOException
	...
	private String parseCCLicense(String str) {
		...
		try {
			...
			org.w3c.dom.Document doc = dBuilder.parse(new ByteArrayInputStream(
					str.getBytes()));
