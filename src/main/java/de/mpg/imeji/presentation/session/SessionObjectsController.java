...
public class SessionObjectsController {
	...
	/**
	 * Remove the item from the {@link List} of selected {@link Item} stored in
	 * the {@link SessionBean}
	 * 
	 * @param itemURI
	 */
	...
	public int removeFromActiveAlbum(List<String> uris) throws ImejiException {
		AlbumController ac = new AlbumController();
		try {
			int deleted = ac.removeFromAlbum(session.getActiveAlbum(), uris,
					session.getUser());
			return deleted;
		} finally {
			reloadActiveAlbum();
	...
	public void reloadActiveAlbum() {
		if (session.getActiveAlbum() != null) {
			...
			session.setActiveAlbum((Album) ic.searchAndSetContainerItems(
					session.getActiveAlbum(), session.getUser(), -1, 0));
		}
	}
}
