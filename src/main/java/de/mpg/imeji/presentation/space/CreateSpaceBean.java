...
public class CreateSpaceBean implements Serializable {
			...
			Collections.sort(collections, new Comparator<CollectionImeji>() {
				...
				public int compare(CollectionImeji coll1, CollectionImeji coll2) {
					return coll1
							.getMetadata()
							.getTitle()
							.compareToIgnoreCase(coll2.getMetadata().getTitle());
				}
			});
		} catch (ImejiException e) {
			BeanHelper.info(sessionBean
					.getMessage("could_not_load_collections_for_space"));
		}
	...
	public List<SelectItem> getCollectionItems() {
		...
		for (CollectionImeji ci : collections)
			itemList.add(new SelectItem(ci.getIdString(), ci.getMetadata()
					.getTitle()));
		return itemList;
	}
	...
	public boolean createdSpace() throws ImejiException, IOException {
		try {
			...
			URI uri = spaceController.create(space, getSelectedCollections(),
				...
				while (iter.hasNext()) {
					...
					if (!fis.isFormField()) {
						try {
							...
							BeanHelper
									.error("Could not process uploaded Logo file streams");
						}
