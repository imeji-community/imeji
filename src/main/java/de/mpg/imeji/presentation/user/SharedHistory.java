...
public class SharedHistory implements Serializable {
	 ...
	 * @param sharedType
	 */
	public SharedHistory(User user, SharedObjectType type, String containerUri,
		...
		this.title = title;
		this.sharedType = ShareBean.initShareMenu(user, null, containerUri,
				profileUri);
	}
	 ...
	 * @param sharedType
	 */
	public SharedHistory(UserGroup group, SharedObjectType type,
			String containerUri, String profileUri, String title) {
		this.setGroup(group);
		this.type = type;
		this.shareToUri = containerUri;
		this.profileUri = profileUri;
		this.title = title;
		this.sharedType = ShareBean.initShareMenu(null, group, containerUri,
				profileUri);
	}
	...
	/**
	 * Update {@link Grants} the {@link SharedHistory} according to the new
	 * roles
	 * 
	 * @return
	 */
	public String update() {
		...
		try {
			ShareBean.share(session.getUser(), user, group, shareToUri,
					profileUri, sharedType);
		} catch (Exception e) {
			logger.error(e);
		}
