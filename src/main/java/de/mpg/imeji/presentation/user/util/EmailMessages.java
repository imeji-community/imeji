...
public class EmailMessages {
	 ...
	 * @return
	 */
	public String getNewPasswordMessage(String password, String email,
			String username) {
		String msg = "";
		try {
			...
			Logger.getLogger(EmailMessages.class).info(
					"Will return empty message, due to some error", e);
			return msg;
		}
	}
	 ...
	 * @return
	 */
	public String getSharedCollectionMessage(String sender, String dest,
			String collectionName, String collectionLink) {
	...
	/**
	 * Email content when an item has been shared with the addressee by the
	 * sender
	 * 
	 * @param sender
	 * @param dest
	 ...
	 * @return
	 */
	public String getSharedItemMessage(String sender, String dest,
			String itemName, String itemLink) {
		String message = getBundle("email_shared_item");
		message = message.replace("XXX_USER_NAME_XXX,", dest)
				.replace("XXX_NAME_XXX", itemName)
				.replace("XXX_LINK_XXX", itemLink)
				.replace("XXX_SENDER_NAME_XXX", sender);
		return message;
	}

	/**
	 * Email content when an album has been shared with the addressee by the
	 * sender
	 * 
	 * @param sender
	 * @param dest
	 ...
	 * @return
	 */
	public String getSharedAlbumMessage(String sender, String dest,
			String collectionName, String collectionLink) {
	...
	private String getEmailOnAccountAction_Body(String password, String email,
		...
		if ("email_new_user".equals(message_bundle)) {
			emailMessage = emailMessage.replace("XXX_LINK_TO_APPLICATION_XXX",
					navigation.getApplicationUrl());
		}
		emailMessage = emailMessage.replace("XXX_USER_NAME_XXX", username)
	 ...
	 * @return
	 */
	public String getEmailOnItemDownload_Body(User to, User actor, Item item,
			CollectionImeji c, SessionBean session) {
		return session
				.getMessage("email_item_downloaded_body")
				.replace("XXX_USER_NAME_XXX", to.getName())
				...
				.replace(
						"XXX_ACTOR_NAME_XXX",
						(actor != null ? actor.getName() : "non_logged_in_user"))
				.replace("XXX_ACTOR_EMAIL_XXX",
						(actor != null ? actor.getEmail() : ""))
				.replace("XXX_TIME_XXX", new Date().toString());
	}

	/**
	 * Generate email subject for "Send notification email by item download"
	 * feature
	 * 
	 * @param item
	 * @param session
	...
	public String getEmailOnItemDownload_Subject(Item item, SessionBean session) {
		return session.getMessage("email_item_downloaded_subject").replace(
				"XXX_ITEM_ID_XXX", item.getIdString());
	}
	 ...
	 * @return
	 */
	public String getEmailOnZipDownload_Body(User to, User actor,
			String itemsDownloaded, String url, SessionBean session) {
		return session
				.getMessage("email_zip_images_downloaded_body")
				.replace("XXX_USER_NAME_XXX", to.getName())
				.replace(
						"XXX_ACTOR_NAME_XXX",
						(actor != null ? actor.getName() : "non_logged_in_user"))
				...
				.replaceAll("XXX_COLLECTION_XXX",
						session.getMessage("collection"))
				.replaceAll("XXX_FILTERED_XXX", session.getMessage("filtered"))
				.replaceAll("XXX_ITEMS_COUNT_XXX",
						session.getMessage("items_count"))
