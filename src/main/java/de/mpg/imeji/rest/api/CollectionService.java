...
public class CollectionService implements API<CollectionTO> {
	...
	private CollectionTO getCollectionTO(CollectionController cc, String id,
			User u) throws ImejiException {
		CollectionTO to = new CollectionTO();
		TransferObjectFactory
				.transferCollection(getCollectionVO(cc, id, u), to);
		return to;
	}

	private CollectionImeji getCollectionVO(CollectionController cc, String id,
			User u) throws ImejiException {
	...
	public CollectionTO create(CollectionTO to, User u) throws ImejiException {
		...
		if (!isNullOrEmpty(profileId)) {
			try {
				mp = pc.retrieve(
						ObjectHelper.getURI(MetadataProfile.class, profileId),
						u);
		...
		collectionURI = cc.create(vo, mp, u,
				cc.getProfileCreationMethod(method), null);
	...
	public CollectionTO update(CollectionTO to, User u) throws ImejiException {
		...
		String hasStatements = originalMp.getStatements().size() > 0 ? " Existing metadata profile has already defined metadata elements. It is not allowed to update it: remove the profileId from your input."
				: "";
		...
		if (!isNullOrEmpty(profileId)) {
			try {
				...
				throw new UnprocessableError(
						"Can not retrieve the metadata profile provided in the JSON body with id: "
								+ profileId + hasStatements);
			}

			if (!profileId.equals(originalMp.getIdString())) {

				if (!METHOD.COPY.toString().equals(method)
						&& !METHOD.REFERENCE.toString().equals(method)) {
					throw new BadRequestException(
							"Wrong metadata profile update method: "
									+ method
		...
		CollectionImeji updatedCollection = cc.updateWithProfile(vo, mp, u,
				cc.getProfileCreationMethod(method));
	...
	@Override
	public CollectionTO withdraw(String id, User u, String discardComment)
			throws ImejiException {
