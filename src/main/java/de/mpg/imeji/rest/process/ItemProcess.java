...
public class ItemProcess {
	...
	public static JSONResponse udpateItem(HttpServletRequest req, String id,
		...
		try {
			// item, no file
			if (file == null) {
				ItemTO to = (ItemTO) RestProcessUtils.buildTOFromJSON(json,
						ItemTO.class);
				validateId(id, to);
				to.setId(id);
				...
				ItemWithFileTO to = (ItemWithFileTO) RestProcessUtils
	...
	private static void validateId(String id, ItemTO to)
		...
		if (!isNullOrEmpty(id) && !isNullOrEmpty(to.getId())
				&& !id.equals(to.getId())) {
			throw new NotFoundException("Ambiguous item id: <" + id
