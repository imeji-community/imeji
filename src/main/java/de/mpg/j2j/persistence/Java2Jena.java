...
public class Java2Jena {
	 ...
	 * @param model
	 * @param lazy
	 *            - defined whether the write operations will be lazy or not
	 *            (i.e. skip or not the {@link List})
	 */
	...
	public void write(Object o) {
		if (J2JHelper.getId(o) == null) {
			throw new NullPointerException("Fatal error: Resource " + o
					+ " with a null id");
		}
		Resource type = model.createResource(J2JHelper.getResourceNamespace(o)
				.toString(), RDFS.Class);
	...
	public void update(String uri, String property, Object obj) {
		...
		if (obj instanceof URI) {
			...
			Literal o = model.createLiteral(
					((LocalizedString) obj).getValue(),
	...
	/**
	 * Add all properties to the {@link Resource} according to the
	 * {@link RDFObject}
	 * 
	 * @param s
	 * @param o
	...
	private void addProperties2Resource(Resource s, Object o) {
		for (Field f : J2JHelper.getAllObjectFields(o.getClass())) {
			try {
				...
				throw new RuntimeException("Error adding property for field "
						+ f + " to object " + o.getClass(), e);
			}
		}
	}
	...
	private void addList2Resource(Resource s, List<?> list, Field f) {
		if (!(lazy && J2JHelper.isLazyList(f))) {
			for (int i = 0; i < list.size(); i++) {
				...
				if (J2JHelper.isResource(listElement)
						&& J2JHelper.getId(listElement) == null) {
					URI ns = URI.create(J2JHelper
							.getResourceNamespace(listElement));
					String objectName = ns.getPath().replaceAll("/terms/", "");
					String subjectId = s.getURI();
					listElement = J2JHelper.setId(listElement,
							URI.create(subjectId + "/" + objectName + "/" + i));
				}
	...
	private void writeResource(Resource s, Object resourceObject) {
		if (J2JHelper.getId(resourceObject) != null) {
			Property p = model.createProperty(J2JHelper
					.getResourceNamespace(resourceObject));
			...
			logger.warn("Can not create resource " + resourceObject
					+ " because of id null");
		}
	}
	...
	private void addURIResource(Resource s, Object resourceURI, Field f) {
		Property p = model.createProperty(J2JHelper.getURIResourceNamespace(
				resourceURI, f));
	...
	/**
	 * Get all Embedded {@link Resource} of an {@link Object} stored in
	 * {@link Jena} as a {@link Resource}
	 * 
	 * @param s
	 *            - the {@link Resource}
	 * @param r
			...
			if (!(lazy && J2JHelper.isLazyList(f))) {
				try {
					...
					if (J2JHelper.isResource(r2) && exists(r2)) {
						Resource o = model.getResource(J2JHelper.getId(r2)
								.toString());
						l.add(o);
																				...
																				// ArrayList<?>)
					{
						...
						Resource resource = model.getResource(J2JHelper
								.getId(r).toString());
						l.add(resource);
						// delete all properties for this predicate
						for (StmtIterator iterator = resource
								.listProperties(model.createProperty(predicate)); iterator
								.hasNext();) {
					...
					throw new RuntimeException(
							"Error getting all embedded resources for " + r, e);
				}
			}
		}
		return l;
