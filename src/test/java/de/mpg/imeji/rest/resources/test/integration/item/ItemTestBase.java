...
public class ItemTestBase extends ImejiTestBase {
  ...
  protected static void initCollectionWithProfile(Collection<Statement> statements)
    ...
    try {
      ...
      collectionTO =
          (CollectionTO) RestProcessUtils.buildTOFromJSON(
              getStringFromPath("src/test/resources/rest/createCollection.json"),
              CollectionTO.class);
      ...
      collectionId =
          ObjectHelper.getId(cc.create(ci, p, JenaUtil.testUser,
  ...
  protected static Collection<Statement> getDefaultBasicStatements() {
    ...
    Statement st;
    for (String type : new String[] {"text", "number", "person", "geolocation", "date", "license",
        "link", "publication"}) {
      st = new Statement();
