...
public class ProfileServiceTest {
  ...
  private static final String DEFAULT_PROFILE_JSON_PATH =
      "src/main/webapp/WEB-INF/default-metadata-profile.json";
  ...
  public void testProfileCRUD_TO() {
    ...
    assertEquals(pro1.getId().toString().split("/")[5], proTO2.getParentStatementId());
    ...
    assertEquals(((ArrayList<LocalizedString>) pro1.getLabels()).get(0).getValue(), proTO1
        .getLabels().get(0).getValue());
    assertEquals(((ArrayList<LocalizedString>) pro2.getLabels()).get(0).getValue(), proTO2
        .getLabels().get(0).getValue());
  ...
  public void testDefaultProfile() throws ImejiException, IOException, URISyntaxException {
    ...
    String path =
        new File(this.getClass().getClassLoader()
            .getResource(ProfileController.DEFAULT_METADATA_PROFILE_PATH_PROPERTY).toURI())
            .getAbsolutePath();
    MetadataProfileTO pFromJSON =
        (MetadataProfileTO) buildTOFromJSON(getStringFromPath(path), MetadataProfileTO.class);
    ...
    MetadataProfileTO pFromService =
        ps.read(ObjectHelper.getId(Imeji.defaultMetadataProfile.getId()), adminUser);

    assertThat(pFromService.getDefault(), equalTo(true));
    assertThat(pFromService.getId(),
        equalTo(ObjectHelper.getId(Imeji.defaultMetadataProfile.getId())));
