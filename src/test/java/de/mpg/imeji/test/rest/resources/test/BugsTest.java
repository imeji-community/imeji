...
public class BugsTest {
  ...
  public void notificationUrlEscapingTest_bug42() {
    ...
    when(Imeji.RESOURCE_BUNDLE.getMessage("email_zip_images_downloaded_body", Locale.ENGLISH))
        .thenReturn(templ);
    when(Imeji.RESOURCE_BUNDLE.getMessage("collection", Locale.ENGLISH)).thenReturn("Collection");
    when(Imeji.RESOURCE_BUNDLE.getMessage("filtered", Locale.ENGLISH)).thenReturn("filtered");
    when(Imeji.RESOURCE_BUNDLE.getMessage("items_count", Locale.ENGLISH)).thenReturn("items count");
